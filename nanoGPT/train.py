...
wandb_project = 't2kg-0'
wandb_run_name = 'gpt2-124M-ft' # 'run' + str(time.time())
# data
dataset = 'openwebtext'
...
max_iters = 811100 # total number of training iterations
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
...
def estimate_loss_and_metrics():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            ...
            losses[k] = loss.item()
            precision, recall, f1 = compute_metrics(logits, Y)
...
while True:
    ...
    # evaluate the loss and metrics on train/val sets and write checkpoints
    if iter_num % eval_interval == 0 and master_process:
        ...
        print(f"step {iter_num}: train loss {metrics['train']['loss']:.4f}, val loss {metrics['val']['loss']:.4f}, " \
              f"val precision {metrics['val']['precision']:.4f}, val recall {metrics['val']['recall']:.4f}, val f1 {metrics['val']['f1']:.4f}")
        if wandb_log:
            wandb.log({
                "iter": iter_num,
